...
while True:
    ...
    frame[mask_red > 0] = [0, 0, 255]     # Pinta de Rojo Fuerte los píxeles detectados (Formato BGR)
    frame[mask_green > 0] = [0, 255, 0]   # Pinta de Verde Fuerte los píxeles detectados (Formato BGR)
...
while indice_elemento < len(elementos_a_ubicar):
    ...
    if contornos_red:
        ...
        if cv2.contourArea(c_red) > 30:
            ...
            if M["m00"] != 0:
                ...
                if cv2.countNonZero(roi_green) > 30: # Ambas baquetas están cruzadas dentro del cuadro
                    if not fijando:
                        if (1, 1) in sonidos and sonidos[(1, 1)]:
                            sonidos[(1, 1)].play()
                        fijando = True
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'):
        pads_game = {
            ...
            (1, 1): (320, 240, 140, 50),  # Caja (Snare)
            (2, 2): (480, 240, 140, 50),  # Tom inferior
...
# Al finalizar la calibración, preparas el mensaje:
lista_elementos = []
for celda, datos in pads_game.items():
    cx, cy, ancho, alto = datos
    pad_data = {"elemento": str(celda), "x": cx, "y": cy, "w": ancho, "h": alto}
...
# 1. Enviar paquete por UDP al middleware
sock.sendto(json.dumps(mensaje_calibracion).encode('utf-8'), (UDP_IP, UDP_PORT))

# 2. GUARDAR CONFIGURACIÓN EN UN ARCHIVO LOCAL
with open("config_pads.json", "w") as f:
    json.dump(mensaje_calibracion, f)
