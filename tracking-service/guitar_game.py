...
METRICS_JSON_PATH = Path(__file__).resolve().parent / "session_metrics_guitar.json"
CREDIT_LINES = [
    "AirDrums",
...
class RhythmGame:
    ...
    def _build_empty_metrics(self):
        # Este cambio inicializa un acumulador único de métricas para toda la sesión actual de guitarra.
        return {
            "hits_outside_game": 0,
            "correct_notes": 0,
            "missed_notes": 0,
    ...
    def _session_elapsed_seconds(self, current_time: float | None = None):
        # Este cambio calcula el tiempo de sesión de guitarra desde que se abrió el juego.
        if current_time is None:
            current_time = pygame.time.get_ticks() / 1000.0
        return max(0.0, current_time - self.session_started_at)

    def _record_hits_outside_game(self, count: int = 1):
        # Este cambio cuenta intentos de rasgueo fuera de una canción activa.
        if self.state == "playing" or count <= 0:
            return
        self.metrics["hits_outside_game"] += count

    def _record_correct_hit_metrics(self, candidate_offset: float):
        # Este cambio acumula notas acertadas de guitarra y su latencia absoluta en milisegundos.
    ...
    def _record_false_hit_metrics(self, candidate_offset: float | None = None):
        # Este cambio acumula rasgueos inválidos y su distancia temporal respecto a la nota más cercana.
    ...
    def _record_missed_note_metrics(self):
        # Este cambio acumula las notas de guitarra que el jugador dejó pasar.
        self.metrics["missed_notes"] += 1

    def _write_metrics_snapshot(self, force: bool = False):
        # Este cambio sobreescribe un único JSON de métricas de guitarra cada cierto tiempo y al cerrar.
        current_time = pygame.time.get_ticks() / 1000.0
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if self.state == "playing" and event.key == pygame.K_SPACE:
                    ...
                    continue
                if event.key in STRUM_DIRECTION_KEYS:
    ...
    def _try_guitar_strum(self):
        ...
        if self.state != "playing":
            if self._current_held_fret_zone() is not None:
                self._record_hits_outside_game()
            return
        if not self._accepts_keyboard_hits():
            return
    ...
    def _activate_pause_menu_option(self, option: str):
        ...
        if option == "Continuar":
            ...
            self._trigger_surrender("restart") # Cambiar por esto
    ...
    def _start_song(self, delay_seconds: float = 3.0, start_epoch: float | None = None):
        # Este cambio marca cada inicio de canción dentro de la sesión de métricas de guitarra.
        self.state = "playing"
