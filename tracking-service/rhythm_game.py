...
POWER_DEFINITIONS = {
    "double_notes": {
        "label": "x2 Notas",
        "color": (255, 176, 92),
        "sequence": ("platillo", "tarola", "tom inferior"),
    },
    "invert_colors": {
        "label": "Invertir",
        "color": (110, 205, 255),
        "sequence": ("hithat", "tom superior", "platillo"),
    },
    "hide_notes": {
        "label": "Parpadeo",
        "color": (186, 124, 255),
        "sequence": ("tarola", "bombo", "hithat"),
    },
}
POWER_PHRASE_LENGTH_DRUMS = 6
...
class RhythmGame:
    ...
    def _handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if self.state == "playing" and event.key in (pygame.K_LSHIFT, pygame.K_RSHIFT):
    ...
    def _rebuild_notes_for_selected_difficulty(self):
        # Este cambio marca frases reales del chart para que otorguen poderes al completarlas.
        ...
        self._assign_power_phrases(self.notes, POWER_PHRASE_LENGTH_DRUMS)
    ...
    def _assign_power_phrases(self, notes: list[Note], phrase_length: int):
        # Este cambio selecciona tramos reales de la canción y los marca como frases de poder.
        for note in notes:
        ...
        for start_index in range(0, len(notes) - phrase_length + 1):
            ...
            if any(note.zone == "bombo" for note in phrase_notes):
                continue
            if phrase_notes[-1].time - phrase_notes[0].time > 6.0:
                continue
        ...
        picker = random.Random(f"{self.song_data.title}|{self.selected_difficulty}|drums")
        selected_starts = []
    ...
    def _reset_power_phrase_progress(self):
        # Este cambio inicializa el progreso de las frases especiales antes de empezar una canción.
        self.power_phrase_states = {}
        for note in self.notes:
    ...
    def _track_power_sequence(self, note: Note):
        # Este cambio verifica si una frase real de poder fue completada exitosamente.
        if note.power_phrase_id is None:
            return
    ...
    def _register_power_phrase_miss(self, note: Note):
        # Este cambio invalida la frase de poder cuando fallas alguna de sus notas especiales.
        if note.power_phrase_id is None:
            return
        state = self.power_phrase_states.get(note.power_phrase_id)
    ...
    def _award_power(self, power_name: str):
        # Este cambio agrega el poder ganado al inventario local y lo refleja visualmente al rival.
        if len(self.available_powers) >= 3:
            self.available_powers.pop(0)
        self.available_powers.append(power_name)
        power_label = POWER_DEFINITIONS[power_name]["label"]
        self._show_command_feedback(f"Poder ganado: {power_label}")

    def _award_random_power(self):
        # Este cambio hace que completar una frase especial entregue un poder aleatorio.
        self._award_power(random.choice(list(POWER_DEFINITIONS.keys())))

    def _use_next_power(self):
        # Este cambio permite lanzar el siguiente poder equipado usando Shift mientras pruebas la partida.
        if self.state != "playing":
            return
        ...
        self._show_command_feedback(f"Poder lanzado: {power_label}")

    def _apply_power_effect(self, power_name: str, current_time: float | None = None):
        # Este cambio activa un debuff temporal en pantalla cuando el rival te lanza un poder.
    ...
    def _cleanup_expired_power_effects(self, current_time: float):
        # Este cambio limpia efectos temporales vencidos para que la pista vuelva a la normalidad.
    ...
    def _has_active_power_effect(self, power_name: str):
        # Este cambio simplifica consultar si un poder está activo sobre la pantalla local.
    ...
    def _draw_health_meter(self, highway_rect: pygame.Rect, side: str, health_value: float, powers: list[str], active_effects: list[str], owner_label: str):
        # Este cambio dibuja barras de vida de ambos jugadores y coloca sus poderes al lado sin tapar la pista.
    ...
    def _draw_power_slots(self, shell_rect: pygame.Rect, side: str, powers: list[str], active_effects: list[str]):
        # Este cambio pinta los poderes ganados y los efectos activos junto a cada barra de vida.
    ...
    def _draw_highway(self, rect: pygame.Rect, preview_time: float, show_song_banner: bool):
        # Este cambio aplica efectos visuales de poderes sobre la pista sin tocar destructivamente el chart base.
